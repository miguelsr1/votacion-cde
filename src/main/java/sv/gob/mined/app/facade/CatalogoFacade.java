...
public class CatalogoFacade {
    ...
    public void guardarUsuarioPadre(Long idPerSiges) {
        Query q = emVotacion.createQuery("SELECT u FROM Usuario u WHERE u.idPerSiges=:pIdPer", Usuario.class);
    ...
    public void guardarDetalleVoto(BigDecimal idParametro, BigInteger idUsuario) {
        ...
        detalleVotaUsuario.setIdParametro(emVotacion.find(ParametroVotacion.class, idParametro));
        detalleVotaUsuario.setIdUsuario(emVotacion.find(Usuario.class, idUsuario));
        detalleVotaUsuario.setFechaInsercion(new Date());
        
        emVotacion.persist(detalleVotaUsuario);
    }
